...
export const addScore = (engine, isPerfect) => {
  ...
  //const currentLivesCount = 2 - engine.getVariable(constant.failedCount)
  const perfect = isPerfect ? lastPerfectCount + 1 : 0;
  //const score = lastGameScore + currentLivesCount * (successScore || 25) + ((perfectScore || 25) * perfect)
  const score =
    perfect > 0
...
export const drawYellowString = (engine, option) => {
  const { string, size, x, y, textAlign } = option;
  const { ctx } = engine;
